...
public class MapReports {
    ...
    // SelectBuilder().addVar(x1).addVar(y1).addVar(x2).addVar(y2).addVar(distance)
    ...
    // .addWhere(node2, Namespace.distance, distance).addWhere(cost,
    // Namespace.distF, List.of(node1, node2))
    ...
    // new Point(soln.getLiteral(x1.getName()).getDouble(),
    // soln.getLiteral(y1.getName()).getDouble()),
    // new Point(soln.getLiteral(x2.getName()).getDouble(),
    // soln.getLiteral(y2.getName()).getDouble()),
    ...
    public static String dumpObstacleDistance(MapImpl map) {
        ...
        for (int i = 0; i < lst.size() - 1; i++) {
            for (int j = i + 1; j < lst.size(); j++) {
                dist[i][j] = lst.get(i).getGeometry().distance(lst.get(j).getGeometry());
                dist[j][i] = dist[i][j];
            }
        }
    ...
    //
    //
    // int minX = points.stream().map(c ->
    // asInt(c.getX())).min(Integer::compare).get();
