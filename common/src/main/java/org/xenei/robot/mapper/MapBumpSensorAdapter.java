	...
	public void accept(BumpSensor.BumpState bumpState) {
		...
		for (BumpSensor.State state : BumpSensor.State.values()) {
			if (bumpState.is(state)) {
				...
				Location relativeStart = scaleInfo.round(Location.from(CoordUtils.fromAngle(thetas.getLeft(), range)));
				Location relativeEnd = scaleInfo.round(Location.from(CoordUtils.fromAngle(thetas.getRight(), range)));
