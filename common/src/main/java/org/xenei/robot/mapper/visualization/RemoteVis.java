...
public class RemoteVis {
	...
	public RemoteVis(Map.VisualizationInitializer initializer) throws IOException {
		this(initializer.map(), initializer.solutionSupplier(), initializer.positionSupplier(),
	...
	class MapHandler implements HttpHandler {
		...
		private void handleGetRequest(HttpExchange httpExchange) throws IOException {
			...
			vizLib.draw(map, solutionSupplier, positionSupplier, targetSupplier).forEach(xmlBuilder::append);
			xmlBuilder.append("</svg></body></html>");
			byte[] result = xmlBuilder.toString().getBytes(StandardCharsets.UTF_8);
			// httpExchange.getResponseHeaders().add("Content-Type", "test/html"); //
			// "image/svg+xml"
	...
	abstract class RemoteDrawingCommand extends VisualizationLibrary.AbstractDrawingCommand {
		...
		protected String points() {
			...
			return String.format("points=\"%s\"", Arrays.stream(coords).map(boundingBox::bound)
					.map(c -> String.format("%f,%f", c.getX(), c.getY())).collect(Collectors.joining(" ")));
		}
	}
	...
	class RemoteVizLib extends VisualizationLibrary<RemoteDrawingCommand> {
		...
		public List<RemoteDrawingCommand> draw(Map map, Supplier<Solution> solutionSupplier,
				Supplier<Position> positionSupplier, Supplier<FrontsCoordinate> targetSupplier) {
				...
				public String toString() {
					if (boundingBox.polygon.intersects(geom)) {
						...
						return String.format(
								"<circle r=\"%f\" cx=\"%f\" cy=\"%f\" stroke=\"%s\" stroke-width=\"3\" fill=\"%s\" />",
		...
		protected RemoteDrawingCommand drawPolygon(Polygon geom, Color color) {
			return new RemoteDrawingCommand(boundingBox, geom, color) {
				...
				public String toString() {
					if (boundingBox.polygon.intersects(geom)) {
						return String.format("<polygon %s style=\"fill:%s;stroke:%s\" />", points(), colorCode,
								colorCode);
					}
					return "";
				}
		...
		protected RemoteDrawingCommand drawLine(Geometry geom, Color color) {
			return new RemoteDrawingCommand(boundingBox, geom, color) {
				...
				public String toString() {
					if (boundingBox.polygon.intersects(geom)) {
						return String.format("<polyline %s style=\"fill:none;stroke:%s;stroke-width=3;\" />", points(),
								colorCode);
					}
					return "";
				}
		...
		protected RemoteDrawingCommand drawString(Geometry geom, Color color) {
			...
			return new RemoteDrawingCommand(boundingBox, geom, color) {
				...
				public String toString() {
					if (boundingBox.polygon.intersects(geom)) {
						Coordinate c = boundingBox.bound(geom.getCoordinate());
						return String.format("<text x=\"%f\" y=\"%f\" fill=\"%s\">%s</text>", c.getX(), c.getY(),
								colorCode, geom.getClass().getSimpleName());
					}
					return "";
				}
			};
