...
public class NavigationSnapshotTest {
    ...
    private static RobutContext.Builder builder = TestingConfiguration.getContextBuilder("NavigationSnapshotTest");

    private static Stream<Arguments> baseParams(BiPredicate<NavigationSnapshot, NavigationSnapshot> test) {
        try (RobutContext context = builder.build()) {
            final Map map = new Map(context, new MapTest.TestingStorage());
