...
public class RpiMover implements Mover, AutoCloseable {
    ...
    private static Options getOptions() {
        return new Options()
                ...
                .addOption(Option.builder("s").type(Double.class).desc("max Speed in meters/minute (min=1, max=150)").hasArg().build())
...
//            mover.ctxt.chassisInfo.radius = newR;
    ...
    public void setHeading(double heading) {
        while (!DoubleUtils.inRange(compass.heading() - heading, 0.01)) {
    ...
    public void makeInternalHeading(double heading) {
        ...
        try (StepMonitor monitor = takeSteps(thetaSteps, -thetaSteps, MAX_RPM)) {
            while (!monitor.complete()) {
                if (DoubleUtils.inRange(compass.instantHeading()-heading, 0.01)) {
                    monitor.stop();
                }
            }
        }
