...
public final class MMC3416xPJ {
	...
	/**
	 * Will reset the sensor by passing a large current through Set/Reset Coil in a
	 * reversed direction
	 */
	public void reset() {
		lock();
	...
	/**
	 * Will disable the charge pump and cause the storage capacitor to be charged
	 * off VDD.
	 */
	...
	static final class Status {
		...
		/**
		 * Indicates the charge pump status, after Refill Cap command, the charge pump
		 * will start running, and this bit will stays high, it will be reset low after
		 * the cap reaches its target voltage and the charge pump is shut off.
		 * 
		...
		@Override
		public String toString() {
			return String.format("Status[measurement:%s pumpOn:%s readDone:%s selfTest:%s]", this.measurementDone(),
					this.pumpOn(), this.readDone(), this.selfTestOk());
		}
	}
