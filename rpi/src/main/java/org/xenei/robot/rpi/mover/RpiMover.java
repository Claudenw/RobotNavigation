...
public class RpiMover extends BaseMover implements AutoCloseable {
            ...
            try (RpiMover mover = new RpiMover(ctxt, null, new Coordinate(0, 0))) {
                ...
                while (true) {
                    ...
                    if (commandLine.hasOption("m")) {
                        List<Double> values = Arrays.stream(commandLine.getOptionValues("m")).map(Double::parseDouble)
                                .toList();
                    ...
                    if (commandLine.hasOption("s")) {
                        List<Integer> values = Arrays.stream(commandLine.getOptionValues("s")).map(Integer::parseInt)
                                .toList();
    ...
    // long steps = Math.round(motor[LEFT].stepsPerRotation() * range /
    // rotationalDistance);
